const { app, Tray, Menu, nativeImage, BrowserWindow, ipcMain, globalShortcut } = require('electron')
const path = require('path');
const {InternetConnection} = require("./src/internetConnection.js")

let tray
let win

const createWindow=()=>{
    win = new BrowserWindow({
        width: 350,
        height: 600,
        show: false,
        titleBarStyle: 'default',
        resizable: true,  
        webPreferences:{
          preload:path.join(__dirname,"src/preload.js")
        }   
      });
      win.loadFile("./src/index.html");
}

app.whenReady().then(() => {
  const icon = nativeImage.createFromDataURL('data:image/png;base64,')
  tray = new Tray(icon)
//  createWindow()

const internetConnection= new InternetConnection()
ipcMain.on("call",(_event,params)=>{internetConnection.sendData(params)})

  const Brightness = Menu.buildFromTemplate([
    { label:"10%",type: "normal",click:()=>{internetConnection.sendData({"dimming":10})}},
    { label:"25%",type: "normal",click:()=>{internetConnection.sendData({"dimming":25})}},
    { label:"50%",type: "normal",click:()=>{internetConnection.sendData({"dimming":50})}},
    { label:"75%",type: "normal",click:()=>{internetConnection.sendData({"dimming":75})}},
    { label:"100%",type: "normal",click:()=>{internetConnection.sendData({"dimming":100})}},
  ])
  const White = Menu.buildFromTemplate([
    { label:"Warm White",type: "normal",click:()=>{internetConnection.sendData({"sceneId":11})}},
    { label:"Daylight",type: "normal",click:()=>{internetConnection.sendData({"sceneId":12})}},
    { label:"Cool White",type: "normal",click:()=>{internetConnection.sendData({"sceneId":13})}},
    { label:"Night Light",type: "normal",click:()=>{internetConnection.sendData({"sceneId":14})}},
  ])
  const Dynamic = Menu.buildFromTemplate([
    { label:"Candlelight",type: "normal",click:()=>{internetConnection.sendData({"sceneId":29})}},
    { label:"Fireplace",type: "normal",click:()=>{internetConnection.sendData({"sceneId":5})}},
    { label:"Fall",type: "normal",click:()=>{internetConnection.sendData({"sceneId":22})}},
    { label:"Club",type: "normal",click:()=>{internetConnection.sendData({"sceneId":26})}},
    { label:"Sunset",type: "normal",click:()=>{internetConnection.sendData({"sceneId":3})}},
    { label:"Romance",type: "normal",click:()=>{internetConnection.sendData({"sceneId":2})}},
    { label:"Party",type: "normal",click:()=>{internetConnection.sendData({"sceneId":4})}},
    { label:"Pastel Colors",type: "normal",click:()=>{internetConnection.sendData({"sceneId":8})}},
    { label:"Spring",type: "normal",click:()=>{internetConnection.sendData({"sceneId":20})}},
    { label:"Summer",type: "normal",click:()=>{internetConnection.sendData({"sceneId":21})}},
    { label:"Forest",type: "normal",click:()=>{internetConnection.sendData({"sceneId":7})}},
    { label:"Jungle",type: "normal",click:()=>{internetConnection.sendData({"sceneId":24})}},
    { label:"Mojito",type: "normal",click:()=>{internetConnection.sendData({"sceneId":25})}},
    { label:"Ocean",type: "normal",click:()=>{internetConnection.sendData({"sceneId":1})}},
    { label:"Deep Dive",type: "normal",click:()=>{internetConnection.sendData({"sceneId":23})}},
    { label:"Pulse",type: "normal",click:()=>{internetConnection.sendData({"sceneId":31})}},
    { label:"Golden White",type: "normal",click:()=>{internetConnection.sendData({"sceneId":30})}},
    { label:"Steampunk",type: "normal",click:()=>{internetConnection.sendData({"sceneId":32})}},
    { label:"Christmas",type: "normal",click:()=>{internetConnection.sendData({"sceneId":27})}},
    { label:"Halloween",type: "normal",click:()=>{internetConnection.sendData({"sceneId":28})}},

  ])
  const color = Menu.buildFromTemplate([
    { label:"Vermilion",type: "normal",click:()=>{internetConnection.sendData({"r":255,"g":75,"b":0,"c":0,"w":54})}},
  ])


  const contextMenu = Menu.buildFromTemplate([
    { label:"ON",type: "normal",click:()=>{internetConnection.sendData({"state":true})}},
    { label:"OFF",type: "normal",click:()=>{internetConnection.sendData({"state":false})}},
    { label:"Brightness", type:"submenu", submenu:Brightness},
    { label:"Dynamic", type:"submenu", submenu:Dynamic},
    { label:"White",type:"submenu",submenu:White},
    // { label:"Get State",type:"normal",click:()=>{internetConnection.getState()}},
    { label:"Color",type:"submenu",submenu:color},
    { label:"Close", type: "normal", click:()=>{app.quit()}},
  ])

//  tray.addListener("click",()=>{win.isVisible() ? win.hide() : win.show()});
  tray.setToolTip('WIZ App')
  tray.setContextMenu(contextMenu)
  tray.addListener("click",()=>{tray.popUpContextMenu()})
  const shortcut=globalShortcut.register("`",()=>{
    tray.popUpContextMenu()
  })

})