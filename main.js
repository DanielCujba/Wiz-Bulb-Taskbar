...
const {sendData} = require("./src/internetConnection.js")

let tray
let win


ipcMain.on("call",(_event,params)=>{sendData(params)})
...
app.whenReady().then(() => {
  ...
  const Brightness = Menu.buildFromTemplate([
    { label:"10%",type: "normal",click:()=>{sendData({"dimming":10})}},
    { label:"25%",type: "normal",click:()=>{sendData({"dimming":25})}},
    { label:"50%",type: "normal",click:()=>{sendData({"dimming":50})}},
    { label:"75%",type: "normal",click:()=>{sendData({"dimming":75})}},
    { label:"100%",type: "normal",click:()=>{sendData({"dimming":100})}},
  ])
